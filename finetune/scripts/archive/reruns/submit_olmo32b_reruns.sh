#!/bin/bash
# Generates and submits one SLURM job per intervention × condition for olmo32b.
...
for INTERV in "${INTERVENTIONS[@]}"; do
    for COND in base lora; do
        ...
        JOB_NAME="o32b-${INTERV:0:2}-${COND}"
        LOG_SLUG="olmo32b_${INTERV}_${COND}"
        TMPSCRIPT=$(mktemp /tmp/slurm_olmo32b_XXXXXX.sh)
