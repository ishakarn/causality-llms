#!/bin/bash
# Generic SLURM template: Llama-3.1-8B-Instruct base + LoRA
# Required env vars (set via --export or parent script):
#   INTERV_NAME  — intervention directory name, e.g. 67_word_replace
#   SPLITS_LIST  — space-separated splits, e.g. "easy hard anticommonsense noncommonsense"

#SBATCH --job-name=interv-llama8b
...
#SBATCH --output=finetune/slurm_logs/%j_interv_llama8b_%x.out
#SBATCH --error=finetune/slurm_logs/%j_interv_llama8b_%x.err
...
echo "=== Done: ${INTERV_NAME} for llama8b ==="
