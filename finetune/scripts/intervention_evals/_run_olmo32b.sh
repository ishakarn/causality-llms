#!/bin/bash
# Generic SLURM template: OLMo-3.1-32B-Instruct base + LoRA
# Required env vars (set via --export or parent script):
#   INTERV_NAME  — intervention directory name
#   SPLITS_LIST  — space-separated splits

#SBATCH --job-name=interv-olmo32b
...
#SBATCH --output=finetune/slurm_logs/%j_interv_olmo32b_%x.out
#SBATCH --error=finetune/slurm_logs/%j_interv_olmo32b_%x.err
...
echo "=== Done: ${INTERV_NAME} for olmo32b ==="
