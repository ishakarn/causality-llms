#!/bin/bash
# Re-run GPT-OSS-20B baseline on all 4 splits (with background in text).
# Required env var: SPLIT

#SBATCH --job-name=base-rerun-gptoss
...
#SBATCH --output=finetune/slurm_logs/%j_base_rerun_gptoss_%x.out
#SBATCH --error=finetune/slurm_logs/%j_base_rerun_gptoss_%x.err
...
echo "  scored: $(python -c "import json; print(f\"{json.load(open('${OUT_DIR}/score/summary.json'))['acc_all']:.3f}\")")"

echo "=== Done: gptoss / ${SPLIT} ==="
