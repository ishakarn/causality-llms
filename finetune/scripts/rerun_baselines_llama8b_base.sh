#!/bin/bash
# Re-run Llama-3.1-8B-Instruct BASE only on one split.
# Required env var: SPLIT

#SBATCH --job-name=base-llama-base
...
#SBATCH --output=finetune/slurm_logs/%j_base_rerun_llama_base_%x.out
#SBATCH --error=finetune/slurm_logs/%j_base_rerun_llama_base_%x.err
...
RUN_ID="llama31-8b-instruct-baseline"
