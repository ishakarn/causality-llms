#!/bin/bash
# Re-run Qwen2.5-3B-Instruct LORA only on one split.
# Required env var: SPLIT

#SBATCH --job-name=base-qwen-lora
...
#SBATCH --output=finetune/slurm_logs/%j_base_rerun_qwen_lora_%x.out
#SBATCH --error=finetune/slurm_logs/%j_base_rerun_qwen_lora_%x.err
