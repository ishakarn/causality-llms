#!/bin/bash
# Submit one job per intervention for olmo32b base + lora on noncommonsense split.
...
for INTERV in "${INTERVENTIONS[@]}"; do
    for COND in base lora; do
        ...
        LOG_SLUG="noncomm_olmo32b_${INTERV}_${COND}"
        TMPSCRIPT=$(mktemp /tmp/slurm_olmo_XXXXXX.sh)

        cat > "$TMPSCRIPT" <<SCRIPT
$COMMON_HEADER
#SBATCH --job-name=nc-o32-${INTERV:0:2}-${COND}
