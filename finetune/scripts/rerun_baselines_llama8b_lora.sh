#!/bin/bash
# Re-run Llama-3.1-8B-Instruct LORA only on one split.
# Required env var: SPLIT

#SBATCH --job-name=base-llama-lora
...
#SBATCH --output=finetune/slurm_logs/%j_base_rerun_llama_lora_%x.out
#SBATCH --error=finetune/slurm_logs/%j_base_rerun_llama_lora_%x.err
